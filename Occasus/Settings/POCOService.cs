...
internal class POCOService : IPOCOService
{
    ...
    public object? AddValue(object? POCO, SettingProperty SettingProperty)
    {
    ...
    public object? ChangeDictionaryItem(object? POCO, PropertyInfo propertyInfo, object? value, object key)
    {
    ...
    public object? ChangeListItem(object? POCO, PropertyInfo propertyInfo, object? value, int index)
    {
        ...
        else if (list is IEnumerable e)
        {
            ...
            if (newlist is not null)
            {
                ...
                if (newlist.GetType().GetMethod("Add", [gType]) is MethodInfo method) //Find the right method
                {
                    ...
                    IEnumerator enumerator = e.GetEnumerator();
                    {
                        ...
                        {
                            method.Invoke(newlist, [index == i ? value : enumerator.Current]);
                            i++;
                        }
    ...
    public object? RemoveValue(object? POCO, SettingProperty SettingProperty, int index)
    {
        ...
        if (SettingProperty.NotNullType.IsCollection())
        {
            ...
            if (list is Array arr)
            {
                ...
                return RemoveFromArray(gType, arr, index);
            }

            if (list is IEnumerable e)
            {

                return RemoveFromEnumerable(propertyInfo.PropertyType, gType, e, index);
            }
        }
    ...
    public object? RemoveValueWithKey(object? POCO, PropertyInfo propertyInfo, object key)
    {
    ...
    private static Array AddToArray(Type gType, IEnumerable arr, object? newValue)
    {
    ...
    {
        var method = list.GetType().GetMethod("Add", [gType]); //Find the right method
        method?.Invoke(list, [NewValueorNewObject(newValue, gType)]);
    ...
    private static Array RemoveFromArray(Type gType, Array arr, int index)
    {
    ...
    private static IEnumerable RemoveFromEnumerable(Type enumerableType, Type itemType, IEnumerable list, int index)
    {
        ...
        if (newlist is not null)
        {
            ...
            var method = newlist.GetType().GetMethod("Add", [itemType]); //Find the right method
